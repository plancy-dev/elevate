...
This document defines **how** we layer external references, tokens, and UI so agents and humans can extend the product consistently. It complements [`README.md`](README.md) (workflow), [`elevate-cal-alignment.md`](elevate-cal-alignment.md), and [`elevate-cursor-alignment.md`](elevate-cursor-alignment.md) (token mapping).
...
[getdesign.md](https://getdesign.md/what-is-design-md) packages similar references (e.g. `npx getdesign@latest add cal`) as drop-in moodboards.

**We use it as:**

| Artifact | Location | Role |
|----------|----------|------|
| **Cal-inspired (project)** | Root [`DESIGN.md`](../../DESIGN.md) | Monochrome / elevation / spacing vocabulary — **§0** states Elevate implementation rules |
| Cal CLI snapshot | [`third-party/cal-getdesign/DESIGN.md`](third-party/cal-getdesign/DESIGN.md) | Vendored output of `npx getdesign@latest add cal` for upstream diff |
| **Mapping** | [`elevate-cal-alignment.md`](elevate-cal-alignment.md) | Cal concepts → `globals.css` tokens (shadows, neutrals; app primary stays blue) |
...
| Cursor mapping | [`elevate-cursor-alignment.md`](elevate-cursor-alignment.md) | Cursor concepts → marketing tokens |
| Upstream index | [awesome-design-md README](https://github.com/VoltAgent/awesome-design-md/blob/main/README.md) | Discover other `DESIGN.md` packs |
...
3. For marketing polish, read root `DESIGN.md` / Cal alignment, then map via `elevate-cursor-alignment.md` or `elevate-cal-alignment.md` as appropriate.
...
| P4 | Reference | ✅ Cal moodboard: root `DESIGN.md` + [`third-party/cal-getdesign/`](third-party/cal-getdesign/) + [`elevate-cal-alignment.md`](elevate-cal-alignment.md). Optional: another pack only after product decision. |
