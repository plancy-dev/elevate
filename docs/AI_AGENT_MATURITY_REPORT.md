...
| **검증 단일 명령** | `pnpm verify` = lint + tsc + unit + build | `package.json` |
...
| **P3** | 품질 게이트 통과 | PR 전 lint·typecheck·test·build | CI `ci.yml` + 로컬 `pnpm verify` |
...
| **T2** | CI와 로컬 정합 | `pnpm verify` ≈ CI job |
...
3. **CI가 핵심 게이트를 커버** — push/PR에 lint, typecheck, unit test, build (`.github/workflows/ci.yml`).  
...
5. **규칙 vs 저장소 설정** — `.cursor` 규칙은 pre-commit·lint-staged를 언급하나, **루트 `package.json`에는 husky/lint-staged가 없음** — 로컬 훅이 별도이거나 문서가 선행한 상태일 수 있어 **정합성 점검 권장**.
...
| CI·게이트 | 15 | **13** | main PR 표준화; verify 정합 |
