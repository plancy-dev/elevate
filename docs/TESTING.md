...
- **`dashboard-access`** — `canUseDashboard` (`dashboard_access` or `role === admin`) via mocked service client
...
2. **Password login → dashboard** (optional but recommended for regression): add to **`.env.local`** (never commit):
   ...
   The E2E user’s **`profiles.dashboard_access`** must be **`true`** *or* **`profiles.role`** must be **`admin`** (and **`SUPABASE_SERVICE_ROLE_KEY`** must be set) or sign-in will land on **`/access-pending`** instead of **`/dashboard`**.
