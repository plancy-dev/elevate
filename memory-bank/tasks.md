...
- **GitHub #62 / #63 (Refs):** #62 선행 — `Dashboard.toc.library` 그룹/항목 라벨 **5 locale 상이** + 회귀 테스트 [`tests/unit/dashboard-toc-library-labels.test.ts`](../tests/unit/dashboard-toc-library-labels.test.ts); CREATIVE [`memory-bank/creative-dashboard-sidebar.md`](memory-bank/creative-dashboard-sidebar.md). #63 — CI에 `pnpm run gstack:check` 단계 추가; 샘플 로그 [`reports/gstack-check-sample.log`](reports/gstack-check-sample.log).
